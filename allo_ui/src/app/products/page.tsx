...
export default function ProductsPage() {
  ...
  const setCurrentReservation = useReservationStore((state) => state.setCurrent);
  ...
  return (
    <AppShell>
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-slate-600 dark:text-slate-300">
            Browse inventory, check warehouse availability, and create a reservation in one step.
          </p>
        </div>
      ...
      {products.data && products.data.length > 0 ? (
          ...
          {products.data.map((product) => (
            <Card key={product.id} className="space-y-4">
              <div>
                ...
                <p className="text-sm text-slate-600 dark:text-slate-300">{product.sku}</p>
                {product.description ? <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{product.description}</p> : null}
              </div>
                  ...
                  <div key={warehouse.warehouseId} className="flex flex-col gap-3 rounded-xl border border-slate-200 p-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-600 dark:text-slate-300">
                        Available: {warehouse.availableQuantity} · Reserved: {warehouse.reservedQuantity}
                      </p>
                    </div>
                    <Button
                      disabled={reserve.isPending || warehouse.availableQuantity < 1}
                      onClick={async () => {
        ...
        <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
          <Card className="space-y-4">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300">
              The catalog is currently empty, so here is a quick overview of what this page will show once inventory is loaded.
            </p>
            <div className="flex flex-wrap gap-3">
              <Button asChild>
          ...
          <div className="space-y-4">
            {emptySpotlight.map((item) => (
              <Card key={item.name} className="space-y-2">
                ...
                <p className="text-sm text-slate-600 dark:text-slate-300">{item.sku}</p>
                <p className="text-sm text-slate-600 dark:text-slate-300">{item.stock}</p>
              </Card>
            ))}
          </div>
        </div>
      ...
      {latestReservationId ? (
        <div className="mt-6">
          <Link href={`/reservations/${latestReservationId}`} className="text-primary underline underline-offset-4">
