  ...
  return (
    <AppShell>
      <Card>
        ...
        {current ? (
          <Link href={`/reservations/${current.id}`} className="text-primary underline">
            Continue reservation {current.id}
          </Link>
        ) : (
          <p className="text-sm">No active in-memory reservation. Reserve one from products page.</p>
        )}
      </Card>
    </AppShell>
  );
