...
const warehouses = [
  ...
  },
  {
    ...
    available: '6,470',
    utilization: 45,
    image: 'https://images.unsplash.com/photo-1494526585095-c41746248156?q=80&w=300&auto=format&fit=crop',
  },
  {
    ...
    available: '5,040',
    utilization: 48,
    image: 'https://images.unsplash.com/photo-1460317442991-0ec209397118?q=80&w=300&auto=format&fit=crop',
  },
];
  ...
  return (
    <AppShell>
      <div className="mb-10 flex flex-col justify-between gap-6 lg:flex-row lg:items-start">
        <div>
          <h1 className="mb-3 text-4xl font-extrabold text-[#16143a] sm:text-5xl">Dashboard</h1>
          <p className="text-lg text-gray-500">Overview of inventory, warehouses and reservations</p>
        </div>
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center lg:gap-6">
          <div className="flex items-center gap-3 rounded-2xl bg-white px-6 py-4 shadow-[0_6px_24px_rgba(0,0,0,0.05)]">
            <CalendarDays className="h-5 w-5 text-[#635bff]" aria-hidden="true" />
            <span className="font-medium text-gray-700">May 12 - May 19, 2025</span>
          </div>
      ...
      <section className="mb-10 grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
        {stats.map((stat) => (
          <div key={stat.label} className="rounded-[28px] bg-white p-7 shadow-[0_6px_24px_rgba(0,0,0,0.05)]">
            <p className="mb-3 text-gray-500">{stat.label}</p>
            <h2 className="mb-2 text-4xl font-bold sm:text-5xl">{stat.value}</h2>
            <span className={stat.direction === 'up' ? 'font-semibold text-green-500' : 'font-semibold text-red-500'}>
      ...
      <div className="grid gap-8 2xl:grid-cols-3">
        <section className="overflow-hidden rounded-[30px] bg-white p-7 shadow-[0_6px_24px_rgba(0,0,0,0.05)] 2xl:col-span-2">
          <div className="mb-8 flex items-center justify-between gap-4">
            ...
            <Link href="/products" className="font-semibold text-[#635bff]">View all</Link>
          </div>
            ...
            <table className="w-full min-w-[760px]">
              ...
              <tbody>
                {warehouses.map((warehouse, index) => (
                  <tr key={warehouse.name} className={index === warehouses.length - 1 ? '' : 'border-b'}>
                    <td className="flex items-center gap-4 py-6 font-medium">
                      <img src={warehouse.image} alt="" className="h-14 w-14 rounded-2xl object-cover" />
                      {warehouse.name}
                    </td>
        ...
        <section className="rounded-[30px] bg-white p-7 shadow-[0_6px_24px_rgba(0,0,0,0.05)]">
          <div className="mb-8 flex items-center justify-between gap-4">
            ...
            <Link href="/reservations" className="font-semibold text-[#635bff]">View all</Link>
          </div>

          <div className="space-y-6">
            {reservations.map((reservation) => (
              <div key={reservation.id} className="flex gap-4 border-b pb-5 last:border-b-0 last:pb-0">
                <img src={reservation.image} alt="" className="h-24 w-24 rounded-2xl object-cover" />
                <div className="min-w-0 flex-1">
                  <div className="flex justify-between gap-3">
                    <div>
                      <h3 className="text-xl font-bold">{reservation.product}</h3>
                  ...
                  <p className="mt-2 text-gray-500">Warehouse: {reservation.warehouse}</p>
                  <p className="mt-3 font-semibold text-red-500">⏰ {reservation.time}</p>
                </div>
              </div>
            ))}
          </div>
