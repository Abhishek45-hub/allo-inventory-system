  ...
  const primaryNav = (
    <nav className="space-y-4" aria-label="Primary navigation">
      {navItems.map((item) => {
        ...
        const active = matchPath === '/' ? pathname === '/' : pathname.startsWith(matchPath);
        return (
          <Link
            key={item.label}
            href={item.href}
        ...
        >
          <div>
            <div className="mb-14 flex items-center justify-between">
              <Link href="/" className="flex items-center gap-3" aria-label="Allo dashboard">
          ...
          <div className="rounded-[30px] bg-[#faf8ff] p-8 text-center">
            ...
            <div className="mx-auto mb-5 flex h-28 w-28 items-center justify-center rounded-[28px] bg-white shadow-[0_6px_24px_rgba(0,0,0,0.05)]">
              <HelpCircle className="h-14 w-14 text-[#635bff]" aria-hidden="true" />
            </div>
            <h3 className="mb-3 text-2xl font-bold">Need help?</h3>
            <p className="mb-6 leading-7 text-gray-500">Check our documentation or contact support.</p>
            <a href="mailto:support@allo.com" className="inline-block rounded-2xl bg-gradient-to-br from-[#6c63ff] to-[#7b61ff] px-7 py-4 font-semibold text-white">
              Contact Support
            </a>
          </div>
        </aside>
